...
def _fetch_ohlcv(ticker, days=60):
    """
    Fetch daily OHLCV from Yahoo Finance for an NGX ticker.
    Returns dict with closes, volumes, or None on failure.
    """
    try:
        url = (
            f"https://query1.finance.yahoo.com/v8/finance/chart/"
            f"{urllib.request.pathname2url(ticker)}"
...
def _fetch_brent_trend():
    """
    Fetch Brent crude 20D vs 60D trend.
    Returns: "UP", "DOWN", or "FLAT"
    """
    try:
        url = (
            "https://query1.finance.yahoo.com/v8/finance/chart/"
            "BZ=F?interval=1d&range=3mo"
        )
