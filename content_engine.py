...
def pick_best_tweet_situation(brief):
    ...
    # Priority 3: Strong FX/Gold signal (70%+ conviction)
    fx_pairs = fx.get("pairs", {}) if fx else {}
    strong_fx = [r for r in fx_pairs.values()
                 if r.get("conviction", 0) >= 70 and r.get("direction") != "NEUTRAL"]
    if strong_fx:
        top_fx = sorted(strong_fx, key=lambda x: x["conviction"], reverse=True)[0]
        return "fx_gold", top_fx

    # Priority 4: Cautious regime or moderate signals
    if regime in ("CAUTIOUS", "CAUTION") or sigs >= 2:
        return "risk_off_macro", macro
    ...
    # Priority 7: Dividend reminder (only on a truly quiet day with no signals)
...
def build_tweet(brief):
    ...
    if situation == "risk_off_with_pick":
        # Best case: RISK_OFF + a strong pick — specific, actionable, context-rich
        mac     = data.get("macro", macro)
        pick    = data.get("pick", {})
        ticker  = pick.get("ticker", "")
        score   = pick.get("score", 0)
        active  = list(mac.get("active_signals", {}).keys())
        top_sig = active[0].replace("_", " ").title() if active else "Risk signals"
        sig2    = (" + " + active[1].replace("_", " ").title()) if len(active) > 1 else ""
        sigs_n  = mac.get("signals_detected", 0)

        options = [
            f"🚨 {sigs_n} risk signals today.\n\n{top_sig}{sig2} — all firing HIGH.\n\n{ticker} is the only name scoring {round(score)}/100 in this environment.\n\n{VOICE['disclaimer']}",
            f"Regime: RISK_OFF.\n\n{top_sig}{sig2}.\n\nIn this environment, I want defensives and energy. System's top pick: {ticker} (score {score}/100).\n\n{VOICE['disclaimer']}",
            f"🚨 Macro is loud today — {sigs_n} signals active.\n\n{top_sig} leading the risk read.\n\n{ticker} is the system's clearest setup right now.\n\n{VOICE['disclaimer']}",
        ]
        tweet = random.choice(options)

    elif situation == "dividend_deadline":
        ticker   = data.get("ticker", "")
        title    = data.get("title", "").replace("💰 ", "").replace("📅 ", "").strip()
        desc     = data.get("desc", "")
        options = [
            f"📅 Dividend deadline.\n\n{title}\n\n{desc[:90]}\n\nMiss the date — miss the income.\n\n{VOICE['disclaimer']}",
            f"Clock ticking on {ticker}.\n\n{desc[:100]}\n\nCalendar drives short-term price action. Know the dates.\n\n{VOICE['disclaimer']}",
        ]
        tweet = random.choice(options)
        ...
        reason = reason[:80]
        ml_line = f" | ML: {ml_prob:.0f}%" if ml_prob > 0 else ""

        options = [
            f"One name catching my eye: ${ticker}\n\nScore: {score}/100{ml_line}\n{sigs} signals aligned.\n\n{reason}.\n\n{VOICE['disclaimer']}",
            f"${ticker} — {sigs} independent signals aligned.\n\nScore: {score}/100{ml_line}\n\n{reason}.\n\nNot a prediction. What the data says.\n\n{VOICE['disclaimer']}",
            f"Data says: ${ticker} ({score}/100).\n\n{reason}.\n\n{sigs} signals aligned — that's when I pay attention.\n\n{VOICE['disclaimer']}",
        ]
        tweet = random.choice(options)
        ...
        driver    = data.get("key_driver", "")[:70]
        conviction= data.get("conviction", 0)
        icon      = "🟢" if direction == "LONG" else "🔴"
        if "Gold" in pair or "XAU" in pair:
            options = [
                f"Gold {icon} {direction} — {conviction}% conviction.\n\nDriver: {driver}.\n\nWhen macro stress is high, gold usually knows first.\n\n{VOICE['disclaimer']}",
                f"🥇 XAU/USD: {direction} ({conviction}%)\n\n{driver}.\n\nGold is the canary. It's singing.\n\n{VOICE['disclaimer']}",
            ]
        else:
            options = [
                f"{pair} — {icon} {direction} ({conviction}% conviction).\n\n{driver}.\n\nWatch the level.\n\n{VOICE['disclaimer']}",
                f"FX signal: {pair} {direction}.\n\n{driver}.\n\nConviction: {conviction}%.\n\n{VOICE['disclaimer']}",
            ]
        tweet = random.choice(options)
        ...
        bullish = [s for s, d in sector_sent.items() if isinstance(d, dict) and d.get("sentiment") == "BULLISH"]
        bearish = [s for s, d in sector_sent.items() if isinstance(d, dict) and d.get("sentiment") == "BEARISH"]
        bull_str = bullish[0].replace("_", " ").title() if bullish else ""
        bear_str = bearish[0].replace("_", " ").title() if bearish else ""
        context = f"\n\n{bull_str} benefiting. {bear_str} taking heat." if bull_str and bear_str else ""

        options = [
            f"{regime_icon} {sigs} risk signals active.\n\n{top_sig}{sig2} — flagged HIGH.{context}\n\nKnow what you own.\n\n{VOICE['disclaimer']}",
            f"Macro is loud — {sigs} signals firing.\n\n{top_sig}{sig2}.\n\nDefensive positioning preferred here.\n\n{VOICE['disclaimer']}",
        ]
        tweet = random.choice(options)
        ...
        options = [
            ...
            f"Regime shift → {regime_name}.\n\n{note}\n\nThis changes how I'm sizing.\n\n{VOICE['disclaimer']}",
        ]
        tweet = random.choice(options)
        ...
        options = [
            f"⚠️ {ticker} earnings {date}.\n\nIf you're holding, have your exit plan ready before the print.\n\nEarnings = vol spike. Always.\n\n{VOICE['disclaimer']}",
            f"Mark the calendar — {ticker} reports {date}.\n\nEarnings without a plan = gambling.\n\nKnow your level. Stick to it.\n\n{VOICE['disclaimer']}",
        ]
        tweet = random.choice(options)
        ...
        if data:
            ...
            reasons = data.get("reasons", [])
            score   = data.get("score", 0)
            note    = reasons[0][:70] if reasons else "building a setup"
            for prefix in ["📰 ", "💰 ", "🚀 ", "✅ ", "📈 "]:
                note = note.replace(prefix, "")
            options = [
                f"Quiet signal day.\n\n${ticker} is the top-scored name right now ({score}/100).\n\n{note}.\n\nNo urgency. Just watching.\n\n{VOICE['disclaimer']}",
                f"Low signal day — not a bad thing.\n\nBest days to do nothing are underrated.\n\n${ticker} on my watch list for a better entry.\n\n{VOICE['disclaimer']}",
            ]
            tweet = random.choice(options)
        else:
...
def _pick_hashtags(situation, brief):
    ...
    elif situation in ("risk_off_macro", "regime_change", "risk_off_with_pick"):
...
def build_thread(brief):
    ...
    # ── Tweet 1: Hook — lead with the most interesting thing, not a data summary ─
    sigs     = macro.get("signals_detected", 0)
    arts     = macro.get("articles_read", 0)
    screened = screen.get("screened", 0)
    top_conv = conv[0] if conv else None
    top_ticker = top_conv.get("ticker", "") if top_conv else ""

    if regime in ("RISK_OFF", "BEAR") or sigs >= 4:
        active_sigs_all = list(macro.get("active_signals", {}).keys())
        sig_preview = active_sigs_all[0].replace("_", " ").title() if active_sigs_all else "Risk signals"
        hook_options = [
        ...
        hook_options = [
            f"1/ {date} brief 🧵\n\nOne name clearing every filter today: ${top_ticker}\n\n{screened}+ stocks screened. Here's the full picture.",
            f"1/ Morning analysis — {date} 🧵\n\n{arts} articles read. {screened}+ stocks ranked.\n\nHere's what matters today.",
            f"1/ Daily brief — {date} 🧵\n\n{sigs} signals detected. {screened}+ stocks screened.\n\nLet's get into it.",
        ]
    else:
        hook_options = [
            f"1/ {date} brief 🧵\n\n{arts} news articles read.\n{screened}+ stocks screened.\nHere's what matters today.",
            f"1/ Daily analysis — {date} 🧵\n\n{sigs} signals detected. {screened} stocks screened.\nThread 👇",
        ]
    threads.append(random.choice(hook_options))
