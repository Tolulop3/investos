...
def bake_dashboard(brief, fx_signals, crypto_signals):
    ...
    if not has_start or not has_end:
        print(f"  ❌ Markers missing — injecting them now as fallback")
        # Fallback: find the BAKED = null line and replace it with markers + data
        fallback_line = "const BAKED = null;"
        if fallback_line in html:
            replacement = (
                f"{MARKER_START}\n"
                f"const BAKED = {baked};\n"
                f"{MARKER_END}"
            )
            html = html.replace(fallback_line, replacement, 1)
            print(f"  ✅ Fallback injection successful")
        else:
            print(f"  ❌ No BAKED line found at all — cannot inject data")
            return False
    else:
    ...
    try:
        with open(dashboard_file, "w", encoding="utf-8") as f:
            f.write(html)
        print(f"  ✅ Dashboard baked successfully ({len(html)//1024}KB)")
        return True
    except Exception as e:
