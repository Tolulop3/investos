...
def apply_score_decay(picks_flat, history):
    """
    Decay conviction boosts for picks that have had no fresh signal activity recently.

    A pick that first appeared 3+ days ago and has NOT improved its score since
    gets a decay penalty applied. This prevents old signals from appearing
    as fresh conviction.

    Rules:
      - 3–6 days stale, score flat or falling  → -5 pts, add STALE flag
      - 7–13 days stale                         → -10 pts, flag
      - 14+ days stale                          → -15 pts, flag + DEGRADING label
      - Score actively rising (score_delta > 0) → no decay (signal still live)
      - Less than 3 days of history             → no decay (too new to judge)

    Modifies picks in-place, returns the same list.
    """
    today = datetime.now().strftime("%Y-%m-%d")
    ...
    for pick in picks_flat:
        ...
        if len(records) < 3:
            continue  # not enough history to assess staleness

        sorted_recs  = sorted(records, key=lambda x: x["date"])
        latest       = sorted_recs[-1]
        oldest_in_w  = sorted_recs[-min(7, len(sorted_recs))]  # up to 7 days back

        score_now   = latest["score"]
        score_7d_ago = oldest_in_w["score"]
        ...
        # Only apply decay if the signal is stale (not freshly rising)
        if score_delta > 2:
            # Score is actively rising — signal is live, no decay
        ...
        if days_in_sys >= 14:
            penalty    = 15
            stale_note = f"⏳ DEGRADING — signal {days_in_sys}d old, score flat/falling"
            pick["stale_label"] = "DEGRADING"
        elif days_in_sys >= 7:
            penalty    = 10
            stale_note = f"⏳ STALE — signal {days_in_sys}d old"
            pick["stale_label"] = "STALE"
        elif days_in_sys >= 3:
            penalty    = 5
            stale_note = f"⏳ Aging signal — {days_in_sys}d old"
            pick["stale_label"] = "AGING"
        else:
            continue  # too fresh to penalise

        if penalty > 0:
            pick["score"]          = max(0, pick["score"] - penalty)
            pick["decay_penalty"]  = penalty
            pick["signal_age_note"] = stale_note

            # Add to pick flags if they exist
            pick_dict = pick.get("pick", {})
            if pick_dict:
                existing_action = pick_dict.get("action", "")
                if stale_note and stale_note not in existing_action:
                    pick_dict["action"] = (existing_action + f" | {stale_note}").strip(" | ")
