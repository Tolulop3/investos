...
def detect_trending_stocks(history, min_days=3, min_rise=10):
    ...
    Anti-staleness: a trend is only shown if the score MOVED in the last 2 days.
    Prevents stocks from appearing as 'trending' for 10+ days on stale data.
    """
    trending_up   = []
    trending_down = []
    breakouts     = []
    ...
    today      = datetime.now().strftime("%Y-%m-%d")
    yesterday  = (datetime.now() - __import__('timedelta', fromlist=['timedelta'])
                  if False else None)
    ...
    for ticker, records in history.items():
        ...
        # Only show trend if the most recent record is within the last 2 days
        # AND the score actually changed recently (not just a frozen baseline)
        most_recent_date = records_sorted[-1]["date"]
        if most_recent_date < cutoff:
            continue  # data is stale — skip this ticker's trend

        # Require the score to have MOVED in the last 2 days
        # (last record different from second-to-last)
        if len(records_sorted) >= 2:
            last_two_delta = records_sorted[-1]["score"] - records_sorted[-2]["score"]
            if abs(last_two_delta) < 1.0:
                continue  # score hasn't moved recently — not a live trend
