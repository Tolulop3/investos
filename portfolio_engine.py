...
CONFIG = {
    ...
    "accounts": {
        ...
        "FHSA": {
            ...
            "balance":          10,        # ← UPDATE when you deposit
...
def apply_sector_cap(picks, max_per_sector=3, elite_bypass_score=88):
    """
    Smart sector cap:
    - ETFs are in their own bucket (never compete with stocks)
    - Stocks scoring elite_bypass_score+ pass regardless of sector cap
    - Max max_per_sector picks per sector for everyone else
    - Result: cleaner shortlist, no good picks left on the floor
    """
    SECTOR_MAP = {
        # Energy
        ...
        "WFC":"FINANCIALS","BLK":"FINANCIALS","BX":"FINANCIALS",
        # Tech
        "SHOP.TO":"TECH","CSU.TO":"TECH","NVDA":"TECH","AMD":"TECH",
        "AAPL":"TECH","MSFT":"TECH","GOOGL":"TECH","META":"TECH","AMZN":"TECH",
        "PLTR":"TECH","CRWD":"TECH","PANW":"TECH","SNOW":"TECH","MDB":"TECH",
        "DDOG":"TECH","CLS.TO":"TECH","KXS.TO":"TECH","LSPD.TO":"TECH",
        # Telecom
        "T.TO":"TELECOM","BCE.TO":"TELECOM","RCI-B.TO":"TELECOM","VZ":"TELECOM",
        # Materials / Gold / Mining
        "ABX.TO":"MATERIALS","WPM.TO":"MATERIALS","AEM.TO":"MATERIALS",
        "K.TO":"MATERIALS","AGI.TO":"MATERIALS","LUN.TO":"MATERIALS","FM.TO":"MATERIALS",
        "NTR.TO":"MATERIALS",
        # Consumer
        "ATD.TO":"CONSUMER","MRU.TO":"CONSUMER","L.TO":"CONSUMER","DOL.TO":"CONSUMER",
        "WMT":"CONSUMER","TGT":"CONSUMER","HD":"CONSUMER","LOW":"CONSUMER",
        "MCD":"CONSUMER","SBUX":"CONSUMER","CMG":"CONSUMER","LULU":"CONSUMER","NKE":"CONSUMER",
        # Healthcare
        "JNJ":"HEALTH","PFE":"HEALTH","MRK":"HEALTH","ABBV":"HEALTH","BMY":"HEALTH",
        "ABT":"HEALTH","AMGN":"HEALTH","MDT":"HEALTH","ISRG":"HEALTH","DXCM":"HEALTH",
        # Utilities
        ...
        # REITs
        "REI-UN.TO":"REITS","HR-UN.TO":"REITS","CAR-UN.TO":"REITS","GRT-UN.TO":"REITS",
        "AP-UN.TO":"REITS","O":"REITS","MAIN":"REITS","STAG":"REITS","VICI":"REITS",
        "AMT":"REITS","PLD":"REITS",
        # Infrastructure / Industrials
        "BN.TO":"INFRA","BAM.TO":"INFRA","CNR.TO":"INFRA","CP.TO":"INFRA",
        "WSP.TO":"INFRA","TIH.TO":"INFRA","STN.TO":"INFRA",
        # Dividends / Income (US)
        "PG":"DIVIDEND","KO":"DIVIDEND","PEP":"DIVIDEND","IBM":"DIVIDEND","MMM":"DIVIDEND",
        # Oil & Gas Services
        "SU.TO":"ENERGY","CVE.TO":"ENERGY",
        # Crypto-adjacent
        "HOOD":"FINTECH","SOFI":"FINTECH","NU":"FINTECH","AFRM":"FINTECH",
    }

    ETF_KEYWORDS = [".TO", "XG", "XE", "XB", "XC", "VFV", "ZCN", "XIU", "XIC",
                    "ZDV", "CDZ", "XDV", "ZEB", "ZRE", "XRE", "HXT", "HXS",
                    "VOO", "VTI", "QQQ", "SPY", "IWM", "VYM", "SCHD", "DVY",
                    "HDV", "DGRO", "XLF", "XLK", "XLE", "XLV", "XLU", "XLRE",
                    "VEA", "EFA", "VGK", "EWG", "EWU", "EWJ", "EWA", "EWC",
                    "VWO", "EEM", "INDA", "FXI", "KWEB", "EWZ", "URTH", "ACWI", "MCHI"]
    ...
    def is_etf(ticker):
        ...
        for kw in ETF_KEYWORDS:
            if t.startswith(kw) or t == kw:
                return True
        # Canadian ETFs typically end in .TO and are 4+ chars
        if t.endswith(".TO") and len(t) >= 6 and t[0] in "XZHV":
            return True
        return False
    ...
    removed       = []
    etf_count     = 0
    max_etfs      = 2  # Max 2 ETFs in any bucket — they dilute individual stock signals

    for pick in picks:
        ticker = pick["ticker"]
        score  = pick.get("score", 0)

        # ETFs: separate bucket, max 2
        if is_etf(ticker):
            if etf_count < max_etfs:
                ...
                removed.append({"ticker": ticker, "sector": "ETF",
                                "reason": f"ETF cap: already {max_etfs} ETFs"})
            continue

        sector = SECTOR_MAP.get(ticker, "OTHER")

        # Elite bypass: score 88+ always passes (system's highest conviction)
        if score >= elite_bypass_score:
            pick["sector"]   = sector
            pick["elite"]    = True
...
# ============================================================
# DAILY SHORTLIST ENGINE
# The 3-pick morning brief you can actually act on
# ============================================================

def build_daily_shortlist(brief, fx_signals=None, win_rate=None):
    """
    From all picks, surface the 3 highest conviction calls:
      - PRIMARY:  single best stock pick to act on today
      - BACKUP:   second best, different sector
      - FX_PLAY:  best FX/Gold signal if conviction >= 65%

    Returns a clean dict ready for dashboard and content engine.
    """
    # Gather all conviction picks
    conviction = brief.get("conviction_picks", [])
    all_picks  = (
        brief.get("screen_results", {}).get("FHSA_top5", []) +
        brief.get("screen_results", {}).get("TFSA_growth_top5", []) +
        brief.get("screen_results", {}).get("TFSA_swing_top3", [])
    )

    # Merge conviction data onto picks
    conv_map = {c["ticker"]: c for c in conviction}
    for p in all_picks:
        t = p["ticker"]
        if t in conv_map:
            p["conviction_signals"] = conv_map[t].get("signals", 1)
            p["conviction_score"]   = conv_map[t].get("score", p.get("score", 0))
        else:
            p["conviction_signals"] = 1
            p["conviction_score"]   = p.get("score", 0)

    # Sort by: conviction signals first, then ML prob, then score
    ranked = sorted(all_picks, key=lambda x: (
        x.get("conviction_signals", 1),
        x.get("ml_prob", 0.5),
        x.get("score", 0)
    ), reverse=True)
    ...
    if deduped:
        ...
        primary = {
            ...
            "signals":     p.get("conviction_signals", 1),
            "price":       d.get("price", 0),
            "day_chg":     d.get("day_chg_pct", 0),
            "perf_30d":    d.get("perf_30d", 0),
            "perf_90d":    d.get("perf_90d", 0),
            "div_yield":   d.get("div_yield", 0),
            "category":    pk.get("category", ""),
            "action":      pk.get("action", "Research and consider entry"),
            "exit_note":   pk.get("exit_note", "Review in 5-7 days"),
            "exp_low":     pk.get("exp_low", 5),
            "exp_high":    pk.get("exp_high", 15),
            "why":         p.get("reasons", [])[:3],
            "watch":       p.get("flags", [])[:2],
            "sector":      p.get("sector", ""),
    ...
    # BACKUP pick — different sector from primary
    backup = None
    primary_sector = primary["sector"] if primary else ""
    for p in deduped[1:]:
        if p.get("sector", "") != primary_sector or p.get("sector", "") == "OTHER":
            d  = p.get("data", {})
            pk = p.get("pick", {})
            backup = {
                "ticker":   p["ticker"],
                ...
                "exp_low":  pk.get("exp_low", 3),
                "exp_high": pk.get("exp_high", 10),
                "why":      p.get("reasons", [])[:2],
                "sector":   p.get("sector", ""),
            }
            break

    # FX PLAY — best signal at 65%+ conviction
    fx_play = None
    if fx_signals:
        fx_list = fx_signals.get("signals", []) if isinstance(fx_signals, dict) else []
        strong_fx = [s for s in fx_list if s.get("conviction_pct", 0) >= 65
                     and s.get("direction") != "NEUTRAL"]
        if strong_fx:
            best_fx = sorted(strong_fx, key=lambda x: x.get("conviction_pct", 0), reverse=True)[0]
            fx_play = {
                "pair":       best_fx.get("pair", ""),
                "direction":  best_fx.get("direction", ""),
                "conviction": best_fx.get("conviction_pct", 0),
                "entry":      best_fx.get("entry", 0),
                "target":     best_fx.get("target", 0),
                "stop":       best_fx.get("stop", 0),
                "rr":         best_fx.get("rr", 0),
                "hold":       best_fx.get("hold_period", ""),
                "driver":     best_fx.get("driver", ""),
            }

    # Win rate context
    win_context = None
    if win_rate and win_rate.get("win_rate"):
        wr = win_rate["win_rate"]
        # Score tier win rate for the primary pick
        tier_key = None
        if primary:
            s = primary["score"]
            if s >= 90:   tier_key = "90-100"
            elif s >= 75: tier_key = "75-89"
            elif s >= 60: tier_key = "60-74"
            else:         tier_key = "below-60"
        tier_wr = win_rate.get("by_score_tier", {}).get(tier_key, {}).get("win_rate") if tier_key else None

        win_context = {
            "overall_win_rate":  wr,
            "total_picks":       win_rate.get("total_resolved", 0),
            "avg_return":        win_rate.get("avg_return", 0),
            "tier_win_rate":     tier_wr,
            "tier":              tier_key,
            "streak":            win_rate.get("streak", 0),
            "streak_type":       win_rate.get("streak_type"),
        }
