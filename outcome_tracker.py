...
def resolve_outcomes(current_prices):
    """
    Check unresolved picks. If 5 trading days (~7 calendar days) have passed,
    resolve WIN/LOSS based on price at that point.
    This gives picks enough time to play out — avoids noise from single-day fluctuations.
    """
    ...
    for o in outcomes:
        ...
        # Resolve after 5 trading days (1 week) — enough time for picks to play out
        # Calendar days ≥ 7 covers 5 trading days including weekends
        if days_passed >= 7:
            ticker      = o["ticker"]
...
def compute_time_weighted_win_rate(resolved):
    """
    Time-weighted win rate — recent picks count more than old ones.

    Why this matters:
    - A 10.6% win rate from 3 months ago may no longer reflect current signal quality.
    - If the system improved last month, a flat average hides that improvement.
    - If regime changed (BULL → BEAR), recent picks reflect current conditions better.

    Method: exponential decay weighting.
    - Each pick gets a weight = e^(-λ * days_ago)
    - λ = 0.02 → half-life of ~35 days (picks from 35 days ago count ~50% as much)
    - Weighted win rate = sum(weight * is_win) / sum(weight)

    Returns dict with:
        tw_win_rate         — time-weighted win rate %
        tw_avg_return       — time-weighted average return %
        tw_resolved_count   — effective sample count (sum of weights, normalized)
        tw_trend            — "IMPROVING" / "STABLE" / "DECLINING"
        tw_30d_win_rate     — win rate on last 30 days only (for comparison)
        tw_90d_win_rate     — win rate on last 90 days only
    """
    import math
    ...
    today = datetime.now().date()
    LAMBDA = 0.02  # Decay constant — half-life ~35 days

    total_weight = 0.0
    weighted_wins = 0.0
    weighted_return = 0.0

    # 30d and 90d slices for trend detection
    last_30 = []
    last_90 = []
    ...
    # 30d and 90d flat win rates for trend comparison
    def flat_wr(picks):
        if not picks:
            return None
        wins = len([p for p in picks if p["outcome"] == "WIN"])
        return round(wins / len(picks) * 100, 1)

    tw_30d = flat_wr(last_30)
    tw_90d = flat_wr(last_90)

    # Trend: compare 30d vs 90d (if we have enough data)
    if tw_30d is not None and tw_90d is not None and len(last_30) >= 3 and len(last_90) >= 5:
        ...
        trend = "BUILDING"  # Not enough data yet
...
def compute_win_rate():
    """
    V2: Compute win rate + expectancy + calibration curve + 30D/60D windows.
    Expectancy = (win_rate × avg_win) - (loss_rate × avg_loss)
    Positive expectancy = real edge. Win rate alone is misleading.
    """
    outcomes = load_outcomes()
    ...
    # ── Expectancy (real edge measurement) ───────────────────────
    ...
    # Expectancy > 0 = real edge. Expectancy < 0 = losing system regardless of win rate.

    # ── Calibration curve (10-point score buckets) ────────────────
    # Reveals actual win rate per score decile — exposes miscalibration
    # If score 90-100 wins less than 60-70, scoring is inverted
    calibration = {}
...
def print_win_rate_report(wr):
    ...
    if wr.get("streak"):
        ...
        streak_label = 'WIN' if wr['streak_type'] == 'WIN' else 'LOSS'
        print(f"\n  {icon} Current streak: {wr['streak']} {streak_label}")
