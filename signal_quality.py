...
def apply_all_signal_quality(picks, bucket=None, verbose=True, include_insider=False):
    ...
    for pick in picks:
        ...
        # V2: Non-linear conflict penalties — one major conflict hurts more
        # Previously linear (8pts) — now non-linear (15pts for major conflicts)
        # One conflict on a 92-score pick should drop it to ~78, not ~84

        # Conflict 1: High RS + Weak/Poor earnings quality
        if rs >= 90 and eq in ("WEAK", "POOR"):
            penalty += 15  # was 8 — major structural conflict
        ...
        # Conflict 4: Negative earnings growth + high conviction
