...
def run_daily(test_mode=False):
    ...
    # ── DAILY SHORTLIST ──────────────────────────────────────────
    # Build the 3-pick morning brief directly from conviction_picks
    # (portfolio_engine.build_daily_shortlist was never implemented)
    try:
        conv = brief.get("conviction_picks", [])

        def _pick_to_shortlist(p):
            pick_inner = p.get("pick", {})
            data_inner = p.get("data", {})
            ml_raw     = p.get("ml_prob", 0.5)
            ml_pct     = round(ml_raw * 100) if ml_raw <= 1 else round(ml_raw)
            return {
                "ticker":      p["ticker"],
                "score":       p.get("score", 0),
                "ml_prob":     ml_pct,
                "signals":     p.get("conviction_count", len(p.get("conviction_signals", []))),
                "signal_list": p.get("conviction_signals", []),
                "price":       data_inner.get("price") or pick_inner.get("price"),
                "stop_loss":   pick_inner.get("stop_loss"),
                "amount":      pick_inner.get("amount"),
                "account":     pick_inner.get("account", "TFSA"),
                "bucket":      pick_inner.get("category", ""),
                "venue":       pick_inner.get("venue", "Questrade"),
                "rs":          p.get("rs_rating"),
                "why":         pick_inner.get("why", ""),
            }

        fx_play = None
        for pair_key, fx_data in (brief.get("fx_signals") or {}).items():
            if isinstance(fx_data, dict):
                conv_val  = fx_data.get("conviction", 0)
                direction = fx_data.get("direction")
                if conv_val >= 65 and direction not in (None, "FLAT", "NEUTRAL"):
                    if fx_play is None or conv_val > fx_play.get("conviction", 0):
                        fx_play = {
                            "pair":       pair_key,
                            "direction":  direction,
                            "conviction": conv_val,
                            "stop_loss":  fx_data.get("stop_loss"),
                            "target":     fx_data.get("target"),
                        }

        shortlist = {
            "primary":     _pick_to_shortlist(conv[0]) if len(conv) > 0 else None,
            "backup":      _pick_to_shortlist(conv[1]) if len(conv) > 1 else None,
            "third":       _pick_to_shortlist(conv[2]) if len(conv) > 2 else None,
            "fx_play":     fx_play,
            "total_picks": len(conv),
            "win_context": brief.get("win_rate"),
        }
        ...
        if shortlist.get("backup"):
            b = shortlist["backup"]
            print(f"  🥈 BACKUP: {b['ticker']} | Score {b['score']} | ML {b['ml_prob']}%")
        if shortlist.get("fx_play"):
            fx = shortlist["fx_play"]
            print(f"  💱 FX PLAY: {fx['pair']} {fx['direction']} ({fx['conviction']}% conviction)")
        print(f"  📋 Shortlist built from {shortlist['total_picks']} conviction picks")
