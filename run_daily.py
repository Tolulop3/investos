...
# ============================================================
# PF DRIFT DRILL-DOWN (attribution only — see PF Drift Monitor in run_daily)
# ============================================================

def _drift_stats(picks):
    """n / win rate / avg return / profit factor for a set of resolved picks."""
    if not picks:
        return {"n": 0, "wr": 0.0, "avg": 0.0, "pf": 0.0}
    wins = [e for e in picks if e.get("outcome") == "WIN"]
    rets = [e.get("actual_return", 0) or 0 for e in picks]
    win_r  = [r for r in rets if r > 0]
    loss_r = [abs(r) for r in rets if r < 0]
    return {
        "n":   len(picks),
        "wr":  round(len(wins) / len(picks) * 100, 1),
        "avg": round(sum(rets) / len(rets), 2),
        "pf":  round(sum(win_r) / sum(loss_r), 2) if loss_r else 0.0,
    }


def compute_alerting_tiers(current_pf, pf_baseline, alert_threshold):
    """Tiers whose drift has crossed the alert threshold in the degrading
    direction (drift <= -alert_threshold). Baseline/threshold are inputs,
    not read from disk here — callers (real run vs tests) supply them."""
    return [
        t for t, base in pf_baseline.items()
        if current_pf.get(t) is not None and (current_pf[t] - base) <= -alert_threshold
    ]


def drift_drilldown(tier_picks, tier, baseline_pf, penalized_tickers, verbose=True):
    """
    Attribution only — no behavior change, no exclusions, no caps, no routing
    changes. Answers: is a tier's PF drift explained by penalized/capped names
    (SCORE_CAP_74 / permanent_exclusions.json / cooldown_flags.json) routing
    into the tier (PENALTY_ROUTING — expected, already-handled), or does it
    persist in the organic names too (GENUINE_DECAY — needs real investigation)?

    `baseline_pf` and `penalized_tickers` are explicit inputs so tests can pass
    a synthetic baseline/ticker set instead of reading real files.

    Returns a dict: {tier, drift, capped_stats, organic_stats, worst5,
    month_pf, verdict, lines} — `lines` is the exact printed text (list of
    strings, one per line, no trailing prints) for callers that want to
    reproduce the output without re-running this function.
    """
    from collections import defaultdict
    from datetime import datetime, timedelta

    cur_stats = _drift_stats(tier_picks)
    cur_pf    = cur_stats["pf"]
    drift     = cur_pf - baseline_pf
    ...
    _p()
    _p(f"  ── DRIFT DRILL-DOWN: {tier} tier (drift={drift:+.2f} ALERT) ──")
    _p()

    capped_by_ticker = defaultdict(list)
    organic_picks    = []
    for e in tier_picks:
        tk = e.get("ticker", "?")
        if tk in penalized_tickers:
            capped_by_ticker[tk].append(e)
        else:
            organic_picks.append(e)

    _p("  CAPPED/PENALIZED NAMES in tier:")
    capped_all = []
    capped_stats = _drift_stats([])
    if capped_by_ticker:
        for tk in sorted(capped_by_ticker):
            cps = capped_by_ticker[tk]
            capped_all.extend(cps)
            cs = _drift_stats(cps)
            _p(f"    {tk:<10} n={cs['n']:>3}  WR={cs['wr']:>5.1f}%  "
               f"avg={cs['avg']:>+5.1f}%  PF={cs['pf']:.2f}")
        capped_stats = _drift_stats(capped_all)
        _p(f"    ── Capped subset PF: {capped_stats['pf']:.2f}")
    else:
        _p("    (none)")
    _p()

    organic_stats = _drift_stats(organic_picks)
    _p("  ORGANIC NAMES in tier (all others):")
    _p(f"    n={organic_stats['n']:>4}  WR={organic_stats['wr']:>5.1f}%  "
       f"avg={organic_stats['avg']:>+5.1f}%  PF={organic_stats['pf']:.2f}")
    _p()

    # WORST 5 organic tickers by PF drag, last 90 days. A ticker with zero
    # losses gets pf=0.0 by the _drift_stats convention (no denominator) —
    # that is NOT a real "worst" signal (means only wins so far), so those
    # tickers are excluded from the ranking rather than sorted to the bottom,
    # which would misrepresent winners as drags.
    cutoff = (datetime.now() - timedelta(days=90)).strftime("%Y-%m-%d")
    organic_recent = [e for e in organic_picks if e.get("signal_date", "") >= cutoff]
    by_ticker = defaultdict(list)
    for e in organic_recent:
        by_ticker[e.get("ticker", "?")].append(e)
    worst5 = sorted(
        ((tk, _drift_stats(ps)) for tk, ps in by_ticker.items()
         if any((p.get("actual_return", 0) or 0) < 0 for p in ps)),
        key=lambda x: x[1]["pf"]
    )[:5]
    _p("  WORST 5 TICKERS by PF drag (last 90 days, organic only):")
    if worst5:
        for tk, s in worst5:
            _p(f"    {tk:<10} n={s['n']:>3}  WR={s['wr']:>5.1f}%  "
               f"avg={s['avg']:>+5.1f}%  PF={s['pf']:.2f}")
    else:
        _p("    (no organic picks in the last 90 days)")
    _p()

    # TIER PF BY MONTH (last 3 calendar months, by signal_date)
    by_month = defaultdict(list)
    for e in tier_picks:
        sd = e.get("signal_date", "")
        if len(sd) >= 7:
            by_month[sd[:7]].append(e)
    months = sorted(by_month)[-3:]
    _p("  TIER PF BY MONTH (last 3 calendar months):")
    month_pf = []
    for i, m in enumerate(months):
        ms = _drift_stats(by_month[m])
        month_pf.append(ms["pf"])
        label = "current" if i == len(months) - 1 else f"Month-{len(months) - 1 - i}"
        _p(f"    {m} ({label}):  PF={ms['pf']:.2f}  n={ms['n']}")
    trend = None
    if len(month_pf) >= 2:
        trend = "IMPROVING" if month_pf[-1] > month_pf[0] else \
                "DECLINING" if month_pf[-1] < month_pf[0] else "STABLE"
        _p(f"    Trend: {trend}")
    _p()

    _p("  Note: capped identification via ticker list membership, not per-pick provenance field")
    verdict = "PENALTY_ROUTING" if organic_stats["pf"] >= baseline_pf else "GENUINE_DECAY"
    if verdict == "PENALTY_ROUTING":
        _p(f"  VERDICT: PENALTY_ROUTING")
        _p(f"  Reason: organic PF {organic_stats['pf']:.2f} >= baseline {baseline_pf:.2f} — capped names are")
        _p(f"  routing into tier and dragging aggregate PF.")
        _p(f"  Action: close watch item.")
    else:
        _p(f"  VERDICT: GENUINE_DECAY")
        _p(f"  Reason: organic PF {organic_stats['pf']:.2f} below baseline {baseline_pf:.2f} — decay is not")
        _p(f"  explained by capped name routing.")
        _p(f"  Action: open new investigation session on organic names.")
    _p()

    return {
        "tier": tier, "drift": drift, "capped_stats": capped_stats,
        "organic_stats": organic_stats, "worst5": worst5, "month_pf": month_pf,
        "trend": trend, "verdict": verdict, "lines": lines,
    }


def load_penalized_tickers():
    """SCORE_CAP_74 (stock_screener.py) union cooldown_flags.json — the real,
    live penalized/capped ticker set used by the actual daily run."""
    try:
        from stock_screener import SCORE_CAP_74 as _score_cap_74
    except Exception:
        _score_cap_74 = set()
    try:
        with open("cooldown_flags.json") as _cdf:
            _cooldowns = json.load(_cdf)
    except Exception:
        _cooldowns = {}
    return set(_score_cap_74) | set(_cooldowns.keys())


def dedupe_top_flat_picks(picks, verbose=True):
    """
    ...
    if brief:
        ...
        try:
            ...
            if _pos.path.exists(_log_path):
                ...
                # ── DRIFT DRILL-DOWN — attribution only, no behavior change ──────
                # Fires only for tiers whose drift crosses the alert threshold in
                # the DEGRADING direction. Answers: is the drop explained by
                # SCORE_CAP_74/permanent_exclusions.json/cooldown_flags.json names
                # routing into this tier (PENALTY_ROUTING — expected, already a
                # known/handled mechanism), or does it persist in the organic
                # (non-capped) names too (GENUINE_DECAY — needs a real investigation)?
                # See drift_drilldown()/compute_alerting_tiers() above for the
                # standalone, independently-testable implementation.
                _alerting_tiers = compute_alerting_tiers(_current_pf, PF_BASELINE, PF_ALERT_THRESHOLD)
                if _alerting_tiers:
                    _penalized_tickers = load_penalized_tickers()
                    for _atier in _alerting_tiers:
                        drift_drilldown(_tier_map.get(_atier, []), _atier,
                                         PF_BASELINE[_atier], _penalized_tickers, verbose=True)
