...
def apply_news_to_screener(screener_results, news_analysis):
    ...
    for bucket in ["FHSA_top5","TFSA_growth_top5","TFSA_income_top5","TFSA_swing_top3",
        ...
        for pick in screener_results.get(bucket, []):
            ...
            if n != 0:
                # V2: Cap news at ±8pts (was ±25). News is a tilt not a driver.
                # +25 was creating fake conviction — stocks scoring 65 hitting 90.
                n_capped = max(-8, min(8, n))
                pick["score"]          = max(0, min(100, pick["score"] + n_capped))
                pick["news_adjustment"]= n_capped
                pick["news_original"]  = n  # track original for analysis
                pick["news_sentiment"] = adj.get("news_sentiment", "NEUTRAL")
                pick["news_reasons"]   = adj.get("reasons", [])
                if n_capped > 0:
                    pick.setdefault("reasons", []).append(f"📰 News +{n_capped}pts: {', '.join(adj.get('reasons',[])[:1])}")
                else:
                    pick.setdefault("flags", []).append(f"📰 News {n_capped}pts: {', '.join(adj.get('reasons',[])[:1])}")
                count += 1
...
def build_conviction_picks(screener_results, x_signals, trends, news_analysis, ml_results):
    ...
    for pick in all_picks:
        ...
        if len(sigs) >= 2:
            ...
            # Momentum must lead — other signals confirm.
            # Prevents news-driven or sentiment-only picks from
            # entering conviction without underlying price strength.
...
def run_daily(test_mode=False):
    ...
    # ── V2: Exposure Enforcement from Sharpe ─────────────────────
    # Risk engine was a decoration — alerts fired but nothing changed.
    # Now Sharpe directly controls capital deployment.
    # This closes the self-correcting loop.
    rolling_sharpe = risk_report.get("decay_monitor", {}).get(
        "rolling_sharpe", {}).get("sharpe", 0) or 0
    if rolling_sharpe >= 0.5:
        system_exposure = 1.0   # Full deployment — edge confirmed
        exposure_label  = "FULL"
        exposure_reason = f"Sharpe {rolling_sharpe:.2f} ≥ 0.5 — edge confirmed"
    elif rolling_sharpe >= 0.0:
        system_exposure = 0.85  # Slight reduction — edge not confirmed
        exposure_label  = "REDUCED"
        exposure_reason = f"Sharpe {rolling_sharpe:.2f} — reduce by 15%"
    elif rolling_sharpe >= -1.0:
        system_exposure = 0.70  # Sharpe negative — defensive posture
        exposure_label  = "DEFENSIVE"
        exposure_reason = f"Sharpe {rolling_sharpe:.2f} < 0 — cut to 70%"
    else:
        system_exposure = 0.50  # Sharpe deeply negative — half exposure
        exposure_label  = "HALF"
        exposure_reason = f"Sharpe {rolling_sharpe:.2f} < -1 — half exposure"
    print(f"  ⚖️  Exposure enforcement: {exposure_label} ({system_exposure*100:.0f}%) — {exposure_reason}")
