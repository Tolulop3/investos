    ...
    try:
        ...
        try:
            ...
            kelly  = compute_kelly_size(
                win_rate       = wr.get("win_rate_pct", 50) / 100,
                avg_win_pct    = wr.get("avg_win_pct",  8.0),
                avg_loss_pct   = wr.get("avg_loss_pct", 5.0),
                account_balance= CONFIG["accounts"]["TFSA"]["balance"],
                regime         = brief.get("market_regime", {}).get("regime", "NORMAL")
                                  if isinstance(brief.get("market_regime"), dict) else "NORMAL",
                resolved_count = wr.get("resolved", 0),
            )
...
def bake_dashboard(brief, fx_signals, crypto_signals):
    ...
    try:
        ...
        with open(dashboard_file, "r", encoding="utf-8") as f:
            verify = f.read()
        if 'const BAKED = {' in verify and '"baked_at"' in verify:
