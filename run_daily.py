...
def bake_dashboard(brief, fx_signals, crypto_signals):
    ...
    try:
        ...
        # Only escape </script> to prevent HTML parser from closing the script tag.
        # Do NOT escape all </ — that corrupts data values and can break JSON parsing.
        baked = baked.replace("</script>", r"<\/script>")
    ...
    if has_start and has_end:
        ...
        # Inject into application/json tag — CSP-safe, no eval needed
        html = html[:s] + f"\nlet BAKED; try {{ BAKED = {baked}; }} catch(e) {{ console.error('BAKED parse error', e); BAKED = null; }}\n" + html[e:]
