...
def build_conviction_picks(screener_results, x_signals, trends, news_analysis, ml_results):
    ...
    # ── CORRELATION FILTER ───────────────────────────────────────
    # Remove picks that are highly correlated (>0.70) with already-selected picks.
    # Sector-proxy: same sector = high correlation assumed.
    # This prevents the brief from being dominated by 5 tech or 5 energy picks.
    try:
        deduped    = []
        used_sectors = {}   # sector → count
        MAX_PER_SECTOR = 2  # at most 2 picks from the same sector in conviction list
        ...
        pass  # If anything goes wrong, keep original list
    ...
    try:
        ...
        try:
            ...
            kelly  = compute_kelly_size(
                ...
                account_balance= CONFIG["accounts"]["TFSA"]["balance"] or 10_000,  # fallback if balance not set
            ...
            print(f"   📐 Kelly sizing: {kelly['recommended_pct']}% per pick "
                  f"(${kelly['recommended_dollars']:,.0f}) — {kelly['method']}")
...
def bake_dashboard(brief, fx_signals, crypto_signals):
    ...
    try:
        ...
        # Escape ALL case variants of </script> — the HTML parser closes the <script>
        # tag on ANY case variant (</script>, </Script>, </SCRIPT>).
        # <\/ is safe in JSON strings (json.loads treats \/ same as /).
        import re as _re
        baked = _re.sub(r'(?i)</script>', r'<\\/script>', baked)
  ...
  if (typeof BAKED !== 'undefined' && BAKED && BAKED.baked_at) {
    console.log('[InvestOS] BAKED loaded OK — baked_at:', BAKED.baked_at);
  } else {
    console.warn('[InvestOS] BAKED is null — showing sample data');
  }
  try {
    loadAll();
  } catch(err) {
    console.error('[InvestOS] loadAll crashed:', err.message, err.stack);
    var bnEl = document.getElementById('bannerNote');
    if (bnEl) bnEl.textContent = 'JS error: ' + err.message + ' — Check console (F12)';
    var btEl = document.getElementById('bannerTitle');
    if (btEl) btEl.textContent = 'RENDER ERROR — data may be missing';
    try {
      BRIEF = SAMPLE_BRIEF; FX = SAMPLE_FX; CONTENT = SAMPLE_CONTENT;
      if (typeof SAMPLE_CRYPTO !== 'undefined') CRYPTO = SAMPLE_CRYPTO;
      renderAll();
    } catch(e2) {
      console.error('[InvestOS] Even sample render failed:', e2.message);
    }
  }
});"""
    if OLD_BOOT in html:
