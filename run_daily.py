...
def bake_dashboard(brief, fx_signals, crypto_signals):
    ...
    if has_start and has_end:
        ...
        html = html[:s] + "\nconst BAKED = (()=>{try{const el=document.getElementById('investos-baked-data');return el?JSON.parse(el.textContent):null;}catch(e){return null;}})();\n" + html[e:]
        # Inject the actual JSON as a <script type="application/json"> tag — CSP safe
        json_tag = f'\n<script type="application/json" id="investos-baked-data">{baked}</script>\n'
        html = html.replace('</body>', json_tag + '</body>', 1)
