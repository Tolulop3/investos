...
def run_daily(test_mode=False):
    ...
    try:
        ...
        # Sort by conviction_count desc, then score desc
        sorted_conv = sorted(conviction,
                             key=lambda x: (x.get("conviction_count", 0), x.get("score", 0)),
                             reverse=True)
        ...
        def make_pick(p):
            ...
            return {
                ...
                "signals":  p.get("conviction_count", 0),  # ← correct key
        ...
        # Best FX play — key is active_calls not calls
        fx_play = None
        fx_active = fx_signals.get("active_calls", []) if isinstance(fx_signals, dict) else []
        if fx_active:
            best_fx = sorted(fx_active, key=lambda x: x.get("conviction", 0), reverse=True)[0]
            fx_play = {
        ...
        if backup:
            print(f"  🥈 BACKUP: {backup['ticker']} | Score {backup['score']} | {backup['signals']} signals")
        if fx_play:
