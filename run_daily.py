...
def bake_dashboard(brief, fx_signals, crypto_signals):
    ...
    if has_start and has_end:
        ...
        # Put JSON data tag BEFORE the BAKED variable reader so element exists when JS runs
        json_tag = f'<script type="application/json" id="investos-baked-data">{baked}</script>'
        html = html[:s] + "\n" + json_tag + "\nconst BAKED = (()=>{try{const el=document.getElementById('investos-baked-data');return el?JSON.parse(el.textContent):null;}catch(e){console.error('BAKED parse error:',e);return null;}})();\n" + html[e:]
        print(f"  ✅ Injected via CSP-safe JSON tag")
    else:
