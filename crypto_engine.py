...
def fetch_whale_alerts(max_alerts=10):
    ...
    try:
        ...
        items = root.findall(".//item")[:max_alerts]
        ...
        for item in items:
            ...
            is_exchange_outflow = any(x in title_lower for x in
                                     ["from binance","from coinbase","from kraken","from okx",
                                      "from bybit","from huobi","from kucoin","from exchange"])
            is_mint             = "mint" in title_lower
            ...
            alerts.append({
                ...
                "coins":     coins_found,
                "sentiment": sentiment,
                "inflow":    is_exchange_inflow,
                "outflow":   is_exchange_outflow,
                "mint":      is_mint,
                "published": pub,
            })
...
def score_whale_alerts(alerts, symbol):
    ...
    bearish = sum(1 for a in coin_alerts if a["sentiment"] == "BEARISH")
    bullish = sum(1 for a in coin_alerts if a["sentiment"] == "BULLISH")
