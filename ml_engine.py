        ...
        if os.path.exists(outcomes_file):
            try:
                ...
                if len(resolved) >= 100:
                    ...
                    for o in resolved:
                        feat = {
                            "momentum_6m":    o.get("perf_90d", 0) / 100,  # 90d as 6m proxy
                            ...
                            "volatility_90d": o.get("volatility", 2) / 100,
                            "beta":           min(o.get("volatility", 2) / 15, 3.0),
                            ...
                            "market_regime":  1 if o.get("market_regime","") == "BULL" else 0,
                            "sector_momentum":0.0,
                        }
                        rows_X.append(feat)
                        rows_y.append(1 if o["outcome"] == "WIN" else 0)

                    X = pd.DataFrame(rows_X)[ML_CONFIG["features"]]
                    y = pd.Series(rows_y, dtype=int)

                    # Cross-sectional: label 1 if return > median of batch
                    returns = [o.get("actual_return", 0) or 0 for o in resolved]
                    med = sorted(returns)[len(returns)//2]
                    y = pd.Series(
                        [1 if (o.get("actual_return") or 0) > med else 0 for o in resolved],
                        dtype=int
                    )
                    print(f"   ✅ Loaded {len(y)} real outcomes | "
                          f"WR: {y.mean():.1%} | Cross-sectional target")
                    return X, y
    ...
    def train(self, verbose=True):
        ...
        if os.path.exists(cache_file):
            try:
                ...
                # Quick test: transform a dummy vector
                import numpy as _np
                ...
                # Cache incompatible (package version mismatch) — delete and retrain
                if verbose:
...
def calculate_position_sizes(picks, portfolio_value, market_regime, current_drawdown=0.0, max_equity=1.0, verbose=True):
    """
