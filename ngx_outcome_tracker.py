...
def resolve_ngx_outcomes(ngx_result):
    """
    Resolve signals that are ≥7 days old.
    Uses current macro state (from today's ngx_result) as exit condition.

    Resolution logic:
      WIN  — regime RISK_ON/NEUTRAL AND score ≥ 60 (macro thesis held)
      LOSS — regime RISK_OFF OR score < 50 (macro thesis failed)
      FLAT — score 50-60 (inconclusive)
    """
    if not ngx_result:
        return 0
    ...
    for o in outcomes:
        ...
        if days_passed < 7:
            continue  # not yet — need full 7 days
        ...
        if score_exit is None:
            # Ticker dropped off the scored list
            o["outcome"]        = "LOSS"
