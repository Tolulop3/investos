...
def score_stock(data, account_type="TFSA_core"):
    ...
    if data["perf_30d"] > 5:    mom = min(35, mom + 13); reasons.append(f"📈 30d momentum: +{data['perf_30d']}%")
    elif data["perf_30d"] > 2:  mom = min(35, mom + 8)
    elif data["perf_30d"] > 0:  mom = min(35, mom + 4)
    elif data["perf_30d"] < -5: mom = max(0, mom - 5);  flags.append(f"⚠️ 30d pullback: {data['perf_30d']}%")
