...
# ── EXCLUSION LIST — delisted, suspended, or structurally broken tickers ──
# These are removed at fetch time to prevent daily error noise.
...
def fetch_ticker_full(ticker, retries=2):
    ...
    for attempt in range(retries):
        try:
            ...
            # --- Price history (12 months daily — needs 200+ days for 200MA breadth) ---
            ...
            # --- Fundamentals ---
            # ETFs lack quoteSummary fundamentals — catch cleanly, use defaults
            # yfinance 1.3.0 prints 404 errors to stderr before raising —
            # suppress stderr during this call to keep logs clean
            import sys, io
            _stderr = sys.stderr
            ...
            # ── Volume Accumulation (up-day vs down-day volume ratio) ──
            # Institutional accumulation signature: volume spikes on up days,
            # quiet on down days. Ratio > 1.4 = smart money accumulating.
            vol_accum_score = 0
            if len(closes) >= 20 and len(volumes) >= 20:
                up_vol = sum(volumes[i] for i in range(max(0,len(closes)-20), len(closes))
                             if closes[i] > closes[i-1])
                dn_vol = sum(volumes[i] for i in range(max(0,len(closes)-20), len(closes))
                             if closes[i] < closes[i-1])
                if dn_vol > 0:
                    ud_ratio = up_vol / dn_vol
                    if   ud_ratio >= 1.8: vol_accum_score = 10
                    elif ud_ratio >= 1.4: vol_accum_score = 6
                    elif ud_ratio >= 1.1: vol_accum_score = 3
                    elif ud_ratio < 0.9:  vol_accum_score = -6  # distribution signal
            ...
            # yfinance inconsistency: some tickers return already-percent (e.g. 2.89),
            # others return decimal (e.g. 0.0289). If raw > 0.30, treat as already %.
            div_yield  = round(raw_dy if raw_dy > 0.30 else raw_dy * 100, 2)
            div_yield  = min(div_yield, 30.0)  # hard cap — no stock yields >30% sustainably
            ...
            rsi_approx = 50.0  # default neutral
            if len(closes) >= 15:
                gains, losses = [], []
            ...
            return {
                ...
                # Near 0 = at high (potentially extended / crowded)
                "pct_from_20d_high": round(
                    (closes[-1] / max(closes[-20:]) - 1) * 100
                    if len(closes) >= 20 else 0, 2
                ),
                "above_ma50":  (closes[-1] > sum(closes[-50:])/50)   if len(closes) >= 50  else False,
                "above_ma200": (closes[-1] > sum(closes[-200:])/200)  if len(closes) >= 200 else False,
            }
...
def score_stock(data, account_type="TFSA_core", strategy_profile=None):
    ...
    # This is the real alpha zone (score 60-74 outperforms 90-100).
    ...
    strong_trend  = above_ma50 and above_ma200
    pullback_zone = -8 <= pct_from_high <= -1  # pulled 1-8% from 20d high
    ...
    # Extension penalty: at/above 20d high = potentially crowded
    if pct_from_high >= 0 and data["perf_30d"] > 10:
        extension_penalty = 4
        flags.append(f"⚠️ Extended: at 20d high + strong 30d momentum")
        pillars["momentum"] = max(0, pillars.get("momentum", mom) - extension_penalty)

    # ── 2. DIVIDEND INCOME (15 pts) — V2: reduced from 20 ──
    # Dividend income matters for FHSA/TFSA income but should not
    # equal momentum weight. A 5% yield with no momentum trend
    # historically underperforms RS leaders.
    div = 0
    dy = data["div_yield"]
    dy_display = round(dy, 2)  # use capped value from fetch
    if dy > 6:    div = 20; reasons.append(f"💰 Exceptional yield: {dy_display}%")
    elif dy > 4:  div = 16; reasons.append(f"💰 Strong yield: {dy_display}%")
    elif dy > 2.5: div = 12; reasons.append(f"💰 Good yield: {dy_display}%")
    elif dy > 1:  div = 7;  reasons.append(f"💰 Dividend: {dy_display}%")
    elif dy > 0:  div = 3

    if 0 < data["days_to_ex_div"] <= 30:
        div = min(15, div + 5)
        reasons.append(f"📅 Ex-div in {data['days_to_ex_div']} days — collect soon!")
    pillars["dividend_income"] = min(15, div)  # V2: cap 15 (was 20)
    ...
    # Volume accumulation bonus — institutional smart money signal
    va = data.get("vol_accum_score", 0)
    if va >= 6:
        reasons.append(f"📈 Volume accumulation — up-day volume dominates")
    elif va <= -6:
        flags.append(f"⚠️ Volume distribution — selling pressure on down days")
    vol_score = max(0, min(10, vol_score + va))
    ...
    # ── SHORT INTEREST RISK ───────────────────────────────
    # High short float = smart money betting against OR squeeze risk
    # Either way: it's a warning flag, not a hard disqualifier
    si = data.get("short_pct_float", 0)
    if si >= 20:
        bonus -= 10
        flags.append(f"🩳 HIGH SHORT INTEREST: {si}% of float sold short — elevated risk")
    elif si >= 10:
        bonus -= 5
    ...
    # Research: Moskowitz, Ooi, Pedersen — 12m return MINUS 1m return.
    # Removes month-end reversal noise from momentum signal.
    # When 30d momentum is negative but 90d is strong: skip-adjusted momentum
    # is weaker than raw 90d suggests. Apply a small penalty.
    perf_90d = data.get("perf_90d", 0) or 0
    perf_30d = data.get("perf_30d", 0) or 0
    skip_adj_momentum = perf_90d - perf_30d  # 90d minus 1m skip
    if skip_adj_momentum < 0 and perf_90d > 5:
        # Recent month reversed — penalise slightly
        skip_penalty = min(5, abs(skip_adj_momentum) * 0.3)
        total = max(0, total - skip_penalty)
        if skip_penalty >= 2:
            flags.append(f"⚠️ Momentum skip: 90d strong but 30d reversing ({perf_30d:+.1f}%)")

    # ── ITEM 6: GEOMETRIC MEAN FOR HIGH-SIGNAL STACKS ────────────────────────
    ...
    # Above 85: each additional raw point = 0.6 calibrated points.
    if total > 85:
        excess   = total - 85
...
def classify_pick(data, score, account_type):
    ...
    return {
        ...
        "short_interest":   data.get("short_pct_float", 0),
...
def run_full_screen(max_tickers=None, verbose=True, strategy_profile=None):
    ...
    try:
        ...
        fhsa_candidates,   _ = apply_sector_cap(fhsa_candidates,   max_sect, regime=regime_now)
    except ImportError:
        pass
    ...
    # ── BREADTH ENGINE ───────────────────────────────────────────────
    # Reuses above_ma50 / above_ma200 already computed per stock
    # No extra API calls — zero cost
    try:
        total = len(raw_data)
        if total > 0:
            n50  = sum(1 for d in raw_data if d.get("above_ma50",  False))
            n200 = sum(1 for d in raw_data if d.get("above_ma200", False))
            p50  = round(n50  / total * 100, 1)
            p200 = round(n200 / total * 100, 1)

            if   p200 >= 70: sig, note = "BROAD_BULL",   "Rally is broad — most stocks above 200MA"
            elif p200 >= 50: sig, note = "MODERATE",     "Moderate breadth — majority above 200MA"
            elif p200 >= 35: sig, note = "NARROW",       "Narrow rally — leadership concentrated"
            else:            sig, note = "BEAR_BREADTH", "Bearish breadth — most stocks below 200MA"

            if   p50 >= 70: mom = "EXPANDING"
            elif p50 >= 50: mom = "NEUTRAL"
            else:           mom = "CONTRACTING"

            # Sector breadth breakdown
            sb = {}
            for d in raw_data:
                sec = d.get("sector", "Other")
                if sec not in sb: sb[sec] = {"t": 0, "a50": 0, "a200": 0}
                sb[sec]["t"]    += 1
                sb[sec]["a50"]  += 1 if d.get("above_ma50",  False) else 0
                sb[sec]["a200"] += 1 if d.get("above_ma200", False) else 0
            sector_breadth = {
                s: {"pct_above_50":  round(v["a50"]  / v["t"] * 100),
                    "pct_above_200": round(v["a200"] / v["t"] * 100),
                    "count": v["t"]}
                for s, v in sb.items() if v["t"] >= 3
            }
            ...
            results["breadth"] = {
                ...
                "signal":           sig,
                "note":             note,
                "momentum_breadth": mom,
                "sector_breadth":   sector_breadth,
            }
            print(f"\n📊 MARKET BREADTH: {p50}% above 50MA | {p200}% above 200MA | {sig}")
        else:
