...
def fetch_ticker_full(ticker, retries=2):
    """
    Pull everything we need to score a stock using yfinance.
    Works from GitHub Actions — handles Yahoo Finance auth automatically.
    """
    try:
    ...
    for attempt in range(retries):
        try:
            ...
            div_yield     = safe("dividendYield", 0) * 100
            div_rate      = safe("dividendRate", 0)
            pe_ratio      = safe("trailingPE") or safe("forwardPE")
            peg_ratio     = safe("pegRatio")
            profit_margin = safe("profitMargins", 0) * 100
            rev_growth    = safe("revenueGrowth", 0) * 100
            earn_growth   = safe("earningsGrowth", 0) * 100
            debt_equity   = safe("debtToEquity")
            roe           = safe("returnOnEquity", 0) * 100
            current_ratio = safe("currentRatio")
            target_price  = safe("targetMeanPrice")
            analyst_rec   = safe("recommendationKey", "")

            ex_div_date = "N/A"; days_to_ex_div = 999
            ...
            try:
                ...
                if fi.fifty_two_week_high: w52_high = fi.fifty_two_week_high
                if fi.fifty_two_week_low:  w52_low  = fi.fifty_two_week_low
            except Exception: pass

            # --- Volume spike detection ---
            # Compare today's volume to 20-day average
            # Spike = 1.5x+ average = unusual institutional activity
            volume_spike      = False
            volume_spike_ratio = 1.0
            if len(volumes) >= 20 and volumes[-1]:
                avg_vol_20 = sum(volumes[-21:-1]) / 20
                if avg_vol_20 > 0:
                    volume_spike_ratio = round(volumes[-1] / avg_vol_20, 2)
                    volume_spike = volume_spike_ratio >= 1.5
...
def score_stock(data, account_type="TFSA_core"):
    ...
    # Volume spike bonus — institutional accumulation signal
    if data.get("volume_spike"):
        ratio = data.get("volume_spike_ratio", 1.0)
        vol_score = min(10, vol_score + 3)
        reasons.append(f"🔥 Volume spike: {ratio}x avg — unusual activity detected")
...
def run_full_screen(max_tickers=None, verbose=True):
    ...
    # Warm up yfinance session before parallel fetching
    # Prevents all workers competing for the same crumb simultaneously
    try:
        import yfinance as _yf_warm
        _w = _yf_warm.Ticker("AAPL")
        _ = _w.fast_info.last_price
        time.sleep(2)
        if verbose: print("  \u21b3 yfinance session warmed up \u2705")
    except Exception:
        pass
