...
# account_preference: "RRSP" | "TFSA" | "FHSA" | "ALL"
ETF_UNIVERSE = [
    # ── CORE EQUITY ──────────────────────────────────────────────────────────
    ("VFV.TO",  "S&P 500 (CAD)",          "CORE",      "core",     "ALL",   None),
    ("XIC.TO",  "TSX Composite",          "CORE",      "core",     "ALL",   "TSX_BROAD"),
    ("XEQT.TO", "All-Equity Global",      "CORE",      "core",     "FHSA",  "TSX_BROAD"),
    ...
    ("ZEB.TO",  "Canadian Banks",         "SECTOR",    "core",     "FHSA",  "CANADIAN_BANKS"),
    ...
    ("ZLB.TO",  "Canadian Low-Vol",       "SECTOR",    "core",     "FHSA",  None),

    # ── THEMATIC — AI / TECH (TFSA/RRSP only — not FHSA) ────────────────────
    ...
    # ── THEMATIC — DEFENSE / WAR (TFSA/RRSP only) ───────────────────────────
    ...
    # ── THEMATIC — EMERGING TECH (TFSA/RRSP only) ───────────────────────────
    ...
    # ── DEFENSIVE / BOND ─────────────────────────────────────────────────────
    ("ZAG.TO",  "Canadian Agg Bonds",     "DEFENSIVE", "core",     "FHSA",  None),
    ("XBB.TO",  "Canadian Bond Universe", "DEFENSIVE", "core",     "FHSA",  None),
    ("TLT",     "US Long Bonds",          "DEFENSIVE", "core",     "RRSP",  None),
    ("ZLB.TO",  "Canadian Low-Vol Eq",    "DEFENSIVE", "core",     "FHSA",  None),
...
# FHSA: conservative Canadian-focused — these tickers are FHSA-preferred
FHSA_PREFERRED  = {"XEQT.TO", "VFV.TO", "XIC.TO", "ZEB.TO", "ZAG.TO",
                   "XBB.TO",  "ZLB.TO", "XEF.TO"}
# FHSA: avoid pure thematic/volatile
FHSA_AVOID      = {"BOTZ","SMH","SKYY","CIBR","ITA","SHLD","PPA",
                   "QTUM","ARKG","BLOK","GLD","TLT","VWO","EEM","XLE","QQQ","VOO"}
...
def run_etf_engine(sector_sentiment, unified_regime, breadth, verbose=True):
    ...
    def top_for_account(acct, n=5):
        """Top N ETFs for a given account type — each account has distinct logic."""
        picks = []
        for e in scored:
            ticker = e["ticker"]

            if acct == "RRSP":
                # RRSP: US-listed preferred (IRS treaty exemption on dividends)
                preferred = not ticker.endswith(".TO") or e["acct_pref"] == "RRSP"

            elif acct == "TFSA":
                # TFSA: .TO preferred, sector + thematic OK, no restriction
                preferred = ticker.endswith(".TO") or e["acct_pref"] == "TFSA"

            else:  # FHSA
                # FHSA: conservative Canadian-focused
                # Prefer: XEQT, VFV, XIC, ZEB, ZAG, ZLB — broad, low-vol, CAD
                # Avoid: all thematic, leveraged, US-only, volatile
                if ticker in FHSA_AVOID:
                    continue   # exclude entirely from FHSA picks
