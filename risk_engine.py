...
def run_decay_monitor(score_history=None, screener_results=None, verbose=True):
    ...
    if neg_alpha_days >= DECAY_ALERT_DAYS and (our_sharpe is None or our_sharpe < 0.0):
        alerts.append({
            "level":   "🚨 CRITICAL",
            "message": f"Strategy has underperformed S&P 500 for {neg_alpha_days} days. "
                       f"Factor may be decaying. Review system.",
        })
    elif neg_alpha_days >= DECAY_ALERT_DAYS and our_sharpe is not None and our_sharpe >= 0.0:
        # Streak exists but Sharpe is positive — system is recovering, note only
        alerts.append({
            "level":   "ℹ️ NOTE",
            "message": f"Alpha vs SPX negative for {neg_alpha_days} days but Sharpe {our_sharpe:.2f} "
                       f"is positive — system recovering, monitor.",
        })
...
def compute_robustness_score(rolling_sharpe, benchmark, momentum_health, neg_alpha_days):
    ...
    # Negative alpha streak — only penalise if Sharpe is also negative
    # If Sharpe is positive, the streak is lag noise, not factor decay
    _sharpe = rolling_sharpe.get("sharpe") if isinstance(rolling_sharpe, dict) else rolling_sharpe
    if _sharpe is None or _sharpe < 0.0:
...
def compute_position_size_guardrail(ticker, account_balance, pick_category,
    ...
    if "SWING" in pick_category.upper():
        ...
        # Swing cap: 2% of balance, min $30, max $200 (mirrors portfolio_engine config)
        max_single = max(30, min(200, round(account_balance * 0.02, 2)))
        ...
        # FHSA: account_balance IS the FHSA balance when called for FHSA picks
        fhsa_bal   = account_balance if account_balance > 0 else 0
        max_single = round(fhsa_bal * 0.25, 2) if fhsa_bal > 0 else 50
