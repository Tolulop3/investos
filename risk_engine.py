...
def run_stress_simulation(screener_results, ml_results=None, verbose=True):
    ...
    # Simulate price slippage from delayed entry (avg 0.3% per day * 2 days)
    DELAY_SLIP_PER_DAY = 0.003
    scenario2_picks = []
    for p in all_picks:
        delay_cost = p["pick"].get("amount", 100) * DELAY_SLIP_PER_DAY * STRESS_DELAY_DAYS * 100  # as pct
        net_exp    = p["exp_high"] - delay_cost
        if net_exp > 1.5:
...
def compute_position_size_guardrail(ticker, account_balance, pick_category,
    ...
    # Load CONFIG safely
    try:
        from portfolio_engine import CONFIG as _CONFIG
    except Exception:
        _CONFIG = {"accounts": {"TFSA": {"max_swing_per_trade": 100}, "FHSA": {"balance": 0}}}

    if "SWING" in pick_category.upper():
        bucket     = buckets["swing"]
        max_single = _CONFIG["accounts"]["TFSA"].get("max_swing_per_trade", 100)
        ...
        fhsa_bal   = _CONFIG["accounts"]["FHSA"].get("balance", 0)
